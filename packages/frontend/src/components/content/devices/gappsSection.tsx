...
const GappsSection = (props: { gappsDetails: MinimalisticDevice }) => {
	const selectGapps = (e: number) => {
		const menulinks = document.getElementsByClassName("buildsinfo"),
			icon = document.querySelectorAll("[id=gappsArrow]");
	...
	return (
		<>
			<div id="gappsdetails" className="hidden">
				{props.gappsDetails &&
				props.gappsDetails.releases &&
				props.gappsDetails.releases.length !== 0 ? (
					props.gappsDetails.releases.map((key, index) => (
						<div
							className="relative text-left w-full my-1"
							key={`gapps-build-${index}`}
						>
							<div className="px-2 my-2 lg:px-6">
								<button
									type="button"
									onClick={() => selectGapps(index)}
									className="
									buildoptions
									relative
									inline-flex
									...
									{props.gappsDetails.releases
										? `${props.gappsDetails.releases[index].version} - ${
												props.gappsDetails.releases[index].type
													.charAt(0)
													.toUpperCase() +
												props.gappsDetails.releases[index].type.slice(1)
										  } - ${new Date(
												props.gappsDetails.releases[index].generatedAt * 1000
										  ).toDateString()}`
										: "None"}
								</button>
											...
											props.gappsDetails.releases[index].images ? (
												<p className="text-base text-blue-500">
													...
													<span className="text-gray-700 dark:text-white">
														...
														{props.gappsDetails.releases[
															index
														].images?.map<ReactNode>(image => (
															<span key={`imgRecoveryBootGapps-${index}`}>
																<br /> -{" "}
																<a href={image.url}>
																	{image.url.substring(
										...
										>
											<div className="flex flex-col md:flex-row">
												<p className="flex place-items-center text-sm text-blue-500">
													...
													{props.gappsDetails.releases
														? new Date(
																props.gappsDetails.releases[index].generatedAt *
																	1000
														  ).toDateString()
														: "None"}
												</p>
												...
												>
													...
													{props.gappsDetails.releases
														? props.gappsDetails.releases[index].url
														: "None"}
												</p>
											</div>
											<div>
